...
a <- 0.4
ouputNAME<-"Cumulative_resp_alpha0.4.csv" 
...
df_total <- data.frame()
resp.table <- data.frame(matrix(nrow = 1, ncol = 7))
colnames(resp.table)<-c('Date', 'RUN', 'SDR_position', 'Lpc', 'Leq' , 'Lz', 'alpha')
...
for(i in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 2:(ncol(Resp.Data)-1)){
    model <- rankLocReg(
      ...
      alpha=a, method="pc", verbose=TRUE)
    ...
    resp.table$RUN <- i # assign the run to the number in the title for the trials completed that day
  ...
  pdf(paste0("C:/Users/samjg/Documents/My_Projects/Geoduck_transgen_offspring_OA/RAnalysis/Data/SDR_data/All_resp_data/plots/",substr(file.names[i], 1,8),"_", "RUN",substr(file.names[i], 14,15),"_",colnames(Resp.Data[j]),"_regression.pdf"))
  plot(model)
  dev.off()
}
}
...
write.table(df_total,ouputNAME,sep=",", row.names=FALSE)
